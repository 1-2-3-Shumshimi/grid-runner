...
function client:update(deltatime)
	...
	self.t = self.t + deltatime -- increase t by the deltatime
	...
	if self.t > self.updaterate then
		...
		dg = string.format("%s %s %f %f", self.entity, 'move', x, y)
		udp:send(dg)	
		...
		-- its appropriate]
		dg = string.format("%s %s $", self.entity, 'update')
		udp:send(dg)
 
		self.t=self.t-self.updaterate -- set t for the next round
	end
